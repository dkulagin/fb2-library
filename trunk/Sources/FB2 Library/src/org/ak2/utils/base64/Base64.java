...
/* ********  E N C O D I N G   M E T H O D S  ******** */    
    ...
    public static String encodeObject( final java.io.Serializable serializableObject, final int options )
    {
        ...
        {
            new JLogMessage(JLogLevel.ERROR, "Unexpected error: ").log(e);
            return null;
        }   // end catch
        finally
        {
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    private static int decode4to3( final byte[] source, final int srcOffset, final byte[] destination, final int destOffset )
    {
        ...
        else
        {
            ...
            return 3;
                ...
                new JLogMessage("{0}: {1}").log(source[srcOffset + 0], DECODABET[source[srcOffset + 0]]);
                new JLogMessage("{0}: {1}").log(source[srcOffset + 1], DECODABET[source[srcOffset + 1]]);
                new JLogMessage("{0}: {1}").log(source[srcOffset + 2], DECODABET[source[srcOffset + 2]]);
                new JLogMessage("{0}: {1}").log(source[srcOffset + 3], DECODABET[source[srcOffset + 3]]);
                return -1;
            } // end catch
        }
    }   // end decodeToBytes
    ...
    public static byte[] decodeFromFile( final String filename )
    {
        ...
        {
            new JLogMessage(JLogLevel.ERROR, "Error decoding from file {0}: ").log(e, filename);
        }   // end catch: IOException
        finally
        {
    ...
    public static String encodeFromFile( final String filename )
    {
        ...
        {
            new JLogMessage(JLogLevel.ERROR, "Error encoding from file {0}: ").log(e, filename);
        }   // end catch: IOException
        finally
        {
            try{ bis.close(); } catch( Exception e) {}
