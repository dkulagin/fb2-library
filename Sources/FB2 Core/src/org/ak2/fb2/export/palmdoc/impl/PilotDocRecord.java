...
public class PilotDocRecord {
    ...
    public void assign(final byte bytes[], final int length) {
        buf = new byte[length];
        System.arraycopy(bytes, 0, buf, 0, length);
    }

    public void assign(final byte bytes[], final int length, final int pos) {
        buf = new byte[length];
    ...
    public int compress() {
        ...
        while (k != i1) {
            ...
            final int i = findPatternBoyerMoore(k, l, j);
        ...
        for (int i2 = k1 = 0; i2 < destIndex;) {
            ...
            final int j2 = destBuf[k1] & 0xff;
            if (j2 >= 128 && j2 < 192) {
                destBuf[++k1] = destBuf[++i2];
            } else if (destBuf[k1] == 1) {
                for (destBuf[k1 + 1] = destBuf[i2 + 1]; i2 + 2 < destIndex && destBuf[i2 + 2] == 1 && destBuf[k1] < 8; i2 += 2) {
                    destBuf[k1]++;
    ...
    public int convertEOL() {
        for (int i = destIndex = 0; i < buf.length;) {
            ...
            /*
             * if (destBuf[destIndex] == 13) if (i < buf.length - 1 && buf[i + 1] == 10) destIndex--; else
    ...
    public int convertEOL(final int i) {
        byte abyte0[] = null;
        if (i == 0) {
    ...
    private int findPatternBoyerMoore(final int i, final int j, int k) {
        final int l = j - i;
        if (l == 0) {
            return k;
        }
        ...
        i1 = l;
        final int k1 = l - 1;
        final int l1 = buf.length;
        do {
            k--;
            for (i1--; buf[k] != buf[i + i1]; i1 = k1) {
                final int i2 = offsets[buf[k] + 128];
                final int j2 = l - i1;
                k += j2 <= i2 ? i2 : j2;
                if (k >= l1) {
                    return -1;
    ...
    private void issue(final byte byte0) {
        final byte byte1 = byte0;
        if (space) {
        ...
        for (int i1 = destIndex = 0; i1 < buf.length;) {
            ...
            if (destBuf[destIndex] == 10) {
                ...
                if (!flag && destIndex - j > i) {
                    if (destBuf[destIndex - 1] == 32 || destBuf[destIndex - 1] == 10) {
                        destIndex--;
                    } else {
                        destBuf[destIndex] = 32;
                    }
